...
const HardwareContainer: React.FC = () => {
    ...
    const handleFileChanged = useCallback(
        (type: 'hardware' | 'block') => async (name: string, path: string) => {
            try {
                if (type === 'hardware') {
    ...
    const handleTextChanged = useCallback(
        (type: 'module' | 'version') => (e: ChangeEvent<HTMLInputElement>) => {
            if (type === 'module') {
    ...
    const handleCreateClicked = useCallback(async () => {
        try {
            ...
            if (
                !(
                    isEmpty(hardwareConfigPath) ||
                    isEmpty(blockFilePath) ||
                    isEmpty(moduleName) ||
                    isEmpty(version)
                )
            ) {
    ...
    return (
        <div className={classes.root}>
            <FileInput value={'HardwareJson'} onFileChanged={handleFileChanged('hardware')} />
            <FileInput value={'BlockFile'} onFileChanged={handleFileChanged('block')} />
            <Grid container direction={'row'}>
                <Grid item xs={6}>
                    <TextField
                        label="ModuleName"
                        value={moduleName}
                        onChange={handleTextChanged('module')}
                ...
                <Grid item xs={6} className={classes.textInput}>
                    <TextField
                        label="Version"
                        value={version}
                        onChange={handleTextChanged('version')}
